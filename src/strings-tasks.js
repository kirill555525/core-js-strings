...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, indx) + str.slice(indx + len);
}
...
function sumOfCodes(str) {
  ...
  if (str !== undefined && str !== null) {
    const lst = [...str];
    total = lst.reduce((one, two) => {
      return one + two.charCodeAt();
    }, 0);
  }
  return total;
}
...
function countVowels(str) {
  const lst = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  return str.split('').filter((a) => lst.includes(a)).length;
}
...
function reverseWords(str) {
  ...
  const b = newStr.map((a) => a.split('').reverse().join('')).join(' ');
  return b;
}
